...
bl_info = {
    ...
    "author": "Philip Lafleur (original author), Henrik Berglund (edits), Sergey Meshkov (edits), SPluzh (edits)",
    "version": (1, 0, 4),
    "blender": (2, 80, 0),
...
class RestoreSymmetry(bpy.types.Operator):
    ...
    source: bpy.props.EnumProperty(
        ...
        items=(
            ('POSITIVE', "Positive side", "Positive side"),
    ...
    def poll(cls, context):
        ...
        return obj and obj.type == 'MESH' and context.mode in {'OBJECT', 'EDIT_MESH', 'SCULPT'}
    ...
    def execute(self, context):
        ...
        if mesh.shape_keys is None:
            ...
            shapekey = context.active_object.active_shape_key.name

        bpy.ops.object.mode_set(mode='OBJECT')

        try:
            moved = restore_symmetry(
                mesh,
                shapekey,
                {'X': 0, 'Y': 1, 'Z': 2}[self.axis],
                self.source,
                self.targetmix
            )
            self.report({'INFO'}, f"Moved vertices: {moved}")
...
def update_verts(v_start, e_start, axis, source, shapelayer, shapekey, targetmix):
    def update_positive(v_right, v_left):
        if(shapekey == "Basis" or shapekey is None): 
            # No shapekeys or Basis shapekey selected - use original code
            ...
            # Shapekeys found - use modified code
...
def restore_symmetry(mesh, shapekey, axis, source, targetmix):
    ...
    if shapekey is None:
        ...
        shapelayer = bm.verts.layers.shape[shapekey]

    tag_central_loops(bm, axis)

    moved_count = 0  # counter for moved vertices

    for e in bm.edges:
        if e.tag:
            e.verts[0].co[axis] = 0.0
            e.verts[1].co[axis] = 0.0
            e.verts[0].tag = True
            e.verts[1].tag = True

    for e in bm.edges:
        if e.tag:
            v_start = starting_vertex(e, axis)
            before_positions = [(v.co.copy(), v) for v in bm.verts]

            update_verts(v_start, e, axis, source, shapelayer, shapekey, targetmix)

            # check which vertices actually moved
            for old_co, v in before_positions:
                if (v.co - old_co).length > 1e-7:
                    moved_count += 1
